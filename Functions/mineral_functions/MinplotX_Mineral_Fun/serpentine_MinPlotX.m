%%serpentine Structural Formula
%last modified 15.07.2024

function [StrctFrm, apfu,options_definition]=serpentine_MinPlotX(data,headers,options)
...
if any(strcmp(headers,'H2O'))
    MC(:,11)=(data(:,strcmp(headers,'H2O'))./H2O_mw).*2; %for H2O
end
...
for c=1:m
    if O2(c,11) > 0 %checks if H2O content is > 0
        O2_N(c,1)=(18)./sum(O2(c,1:11),2); %hydrous normalization
    else
        O2_N(c,1)=(14)./sum(O2(c,1:10),2); %anhydrous normalization
    end
end
...
for c=1:m
    if O2(c,11) > 0 %checks if H2O content is > 0
        apfu(c,11) = N_Ox(c,11).*2; %H, %hydrous normalization
    else
        apfu(c,11) = 8; %fixes H the value to 8
    end
end

apfu(:,12)=sum(apfu(:,1:10),2); %calculations the cation total
...
StrctFrm(:,14)=apfu(:,12); %Sum
StrctFrm(:,15)=apfu(:,11); %OH

%limit on significant digits (eliminates rounding noise)
StrctFrm(StrctFrm<1e-6) = 0;
apfu(apfu<1e-6) = 0;

StrctFrm=array2table(StrctFrm,'VariableNames',{'Si_T','Al_T','Fe3_T','T_Sum','Al_M','Ti_M','Cr_M','Ni_M','Fe3_M','Fe2_M','Mn_M','Mg_M','Ca_M','Cation_Sum','OH_A'});
apfu=array2table(apfu,'VariableNames',{'Si','Ti','Al','Cr','Ni','Fe3','Fe2','Mn2','Mg','Ca','OH','Cation_Sum'});
