...
function [StrctFrm, apfu,options_definition]=epidote_MinPlotX(data,headers,options)
...
elseif any(strcmp(headers,'FeO')) || any(strcmp(headers,'Fe2O3'))
    ...
    %If no Fe2O3 and FeO exist, nothing happens
end
...
elseif any(strcmp(headers,'MnO')) || any(strcmp(headers,'Mn2O3'))
    ...
    else %if Mn2O3 is Mn2O3 total

        %first calculate the weight percent of Mn2O3 and MnO from Mn2O3 total
    ...
    %If no Mn2O3 and MnO exist, nothing happens
end
...
O2(:,14)=MC(:,14).*(1/2); %for H2O

if any(strcmp(headers,'H2O'))
    O2_N=(13)./sum(O2,2); %hydrous normalization
else
    O2_N=(12.5)./sum(O2,2); %anhydrous normalization
end 
...
for c=1:m
    if O2(c,14) > 0 %checks if H2O content is > 0
        ...
        apfu(c,14) = 1; %fixes H the value to 8
    end
end
...
%M sites - Ti, Al, Cr3+, Fe3+, Mn3+, Mg2+, Fe2+, Mn2+
%A site - Ca, Sr, Na, K, Mg, Fe2+, Mn2+ (Sums to 2)
...
M_rem = 3 - sum(StrctFrm(:,4:1:8),2); % remaining space on M after trivalent cations

XFe=apfu(:,7)./sum(apfu(:,7:1:9),2); %Mn / (Mg + Fe2 + Mn2)
XMn=apfu(:,8)./sum(apfu(:,7:1:9),2); %Mg / (Mg + Fe2 + Mn2)
XMg=apfu(:,9)./sum(apfu(:,7:1:9),2); %Fe2 / (Mg + Fe2 + Mn2)
...
for c=1:m
    if M_rem(c,1) > 0
        if M_rem(c,1) >= sum(apfu(c,7:1:9),2)
            %if the remaining space on M is ≥ total Fe2, Mn2, and Mg
            %all Fe2, Mn2, and Mg are assigned to M
            StrctFrm(c,9)=apfu(c,7); %Fe2(M)
            StrctFrm(c,10)=apfu(c,8); %Mn2(M)
            StrctFrm(c,11)=apfu(c,9); %Mg(M)
        else
            %if there is extra Mg, Fe2, and Mn2, these elements will be
            %equipartitioned between M and A
            StrctFrm(c,9)=M_rem(c,1).*XFe(c,1); %Fe2(M)
            StrctFrm(c,10)=M_rem(c,1).*XMn(c,1); %Mn2(M)
            StrctFrm(c,11)=M_rem(c,1).*XMg(c,1); %Mg(M)
        end
    else
...
%A site 
StrctFrm(:,13)=apfu(:,7)-StrctFrm(:,9); %Fe2 (A) = Fe2 total - Fe2 (M)
StrctFrm(:,14)=apfu(:,8)-StrctFrm(:,10); %Mn2 (A) = Mn2 total - Mn2 (M)
StrctFrm(:,15)=apfu(:,9)-StrctFrm(:,11); %Mg (A) = Mg total - Mg (M)
...
StrctFrm(:,21)=apfu(:,14); %OH

%endmembers
Endmembers(:,1)=(StrctFrm(:,4)-2)./(StrctFrm(:,4)+apfu(:,4)+apfu(:,5)+apfu(:,6)-2); %XCzo = (Al-2)/(Fe3 + Al3 + Cr3 + Mn3 -2)
Endmembers(:,2)=(apfu(:,5))./(StrctFrm(:,4)+apfu(:,4)+apfu(:,5)+apfu(:,6)-2); %XEp = (Fe3)/(Fe3 + Al3 + Cr3 + Mn3 -2)
Endmembers(:,3)=(apfu(:,6))./(StrctFrm(:,4)+apfu(:,4)+apfu(:,5)+apfu(:,6)-2); %Xpmt = (Mn3)/(Fe3 + Al3 + Cr3 + Mn3 -2)
Endmembers(:,4)=(apfu(:,4))./(StrctFrm(:,4)+apfu(:,4)+apfu(:,5)+apfu(:,6)-2); %XTaw = (Cr3)/(Fe3 + Al3 + Cr3 + Mn3 -2)
...
StrctFrm=array2table(all,'VariableNames',{'Si_T','Al_T','Sum_T','Al_M','Ti_M','Cr_M','Fe3_M','Mn3_M','Fe2_M','Mn2_M','Mg_M','Sum_M','Fe2_A','Mn2_A','Mg_A','Ca_A','Sr_A','Na_A','K_A','Sum_A','OH','Xczo','Xep','Xpmt','XCrep'});
apfu=array2table(apfu,'VariableNames',{'Si','Ti','Al','Cr3','Fe3','Mn3','Fe2','Mn2','Mg','Ca','Sr','Na','K','OH','Cation_Sum'});
