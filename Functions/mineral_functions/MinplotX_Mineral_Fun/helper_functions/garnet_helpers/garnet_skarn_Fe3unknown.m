...
function [StrctFrm, apfu]=garnet_skarn_Fe3unknown(data,headers,options)
...
for c=1:m
    if Ti_Scho(c,1)>0 %Is Ti via schorlomite > 0? If y, then some Fe3+ goes into T
        ...
        StrctFrm(c,3)=0; %if Si+Al=3, then no Fe3+ goes into T
    end
end
...
Si_Tet=StrctFrm(:,1)./3; %fraction of Si on tetrahedral site
Schorl=(1-Si_Tet).*(3/2); %schorlomite + Al garnet
Si_Grt=1-Schorl; %total fraction of Si=3 garnet endmembers 
...
for c=1:m
    if StrctFrm(c,16)+StrctFrm(c,15)+StrctFrm(c,17)+StrctFrm(c,18) > 0
        Alm_u(c,1)=(StrctFrm(c,16)./(StrctFrm(c,16)+StrctFrm(c,15)+StrctFrm(c,17)+StrctFrm(c,18))).*AlGrt(c,1); %almandine, un-normalized
        Py_u(c,1)=(StrctFrm(c,15)./(StrctFrm(c,16)+StrctFrm(c,15)+StrctFrm(c,17)+StrctFrm(c,18))).*AlGrt(c,1); %pyrope, un-normalized
        Sps_u(c,1)=(StrctFrm(c,17)./(StrctFrm(c,16)+StrctFrm(c,15)+StrctFrm(c,17)+StrctFrm(c,18))).*AlGrt(c,1); %spessartine, un-normalized
        Grs_u(c,1)=(StrctFrm(c,18)./(StrctFrm(c,16)+StrctFrm(c,15)+StrctFrm(c,17)+StrctFrm(c,18))).*AlGrt(c,1); %grossular, un-normalized
    else
        Alm_u(c,1)=0; %almandine, un-normalized
        Py_u(c,1)=0; %pyrope, un-normalized
        Sps_u(c,1)=0; %spessartine, un-normalized
        Grs_u(c,1)=0; %grossular, un-normalized
    end
end
...
StrctFrm=array2table(all,'VariableNames',{'Si_T','Al_T','Fe3_T','T_Sum','Si_Oct','Al_Oct','Ti_Oct','Cr_Oct','Fe3_Oct','Mg_Oct','Fe2_Oct','Mn2_Oct','Oct_Sum','Y_Dod','Mg_Dod','Fe2_Dod','Mn2_Dod','Ca_Dod','Na_Dod','Dod_Sum','Xslo','Xmmt','Xadr','Xuv','Xalm','Xprp','Xsps','Xgrs','O2_deficiency'});
