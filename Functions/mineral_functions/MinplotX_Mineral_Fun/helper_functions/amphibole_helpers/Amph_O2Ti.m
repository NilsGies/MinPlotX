function [O2_Ti,apfu_I,MC]=Amph_O2Ti(D,W,TiOH_correction,H2Oknown)
...
H24=24.*(MC(:,12)./Anion_Sum); %hydrogens per 24 anions
O_min=24-0.5*H24-0.5*F_Cl; %Minimum O equivalents

%Ti per 24 anions correction
Ti_adj(:,1)=zeros(m,1);
for c=1:m 
...
%correction for OH=2-2Ti (or not), max O equivalents
if strcmp(TiOH_correction, 'y')
...
if strcmp(H2Oknown, 'y')
    O2_N=O_min./Anhydrous_Sum;
    % O2_Ti=O_min;
else
